...
class Solution:
    ...
    def check_possibility(self):
        ...
        """
        # 假设有array为：...i-2,i-1,i,i+1,... ，需要做调整的情况有几种： 秉持一个原则，在符合要求的前提下，数字越小越容易满足要求
        # 1、考虑array为： ...1, 4, 2, 3,... 有两种方法，a). 4 -> 2 正解，因为对于i的位置来说，在满足非递减原则下，其值越小越好，越容易满足i+1及其后面非递减的原则；b). 2 -> 4
        # 2、考虑array为： ...3, 4, 2, 3,... 只有一种方法，a). 2 -> 4 (虽然调整后i-1,i,i+1也不能满足非递减的原则，但是i-2,i-1,i三个元素必然需要调整一次值，所以该例子并不能在改变1个元素的情况下，使之变成一个非递减数列)
